...
MVPでは完全な公式ブラケット再現よりも、推し国視点の表示を優先する。
...
```txt
決勝トーナメントの組み合わせは未確定です。
グループ最終順位と3位通過チームの組み合わせにより変動します。
```

## 8.5 後続実装

正式なトーナメント組み合わせが確定したら、以下を追加する。
...
- 既存のグループ順位計算結果から、現在1位・2位をR32の型付き枠へ差し込む。
- 3位通過枠はチームを割り当てず、`3位通過枠 / 未確定` として扱う。
- R16以降は前ラウンドの勝者枠を接続し、勝者や勝ち上がりを予想しない。
- メイン推し国が暫定R32に含まれる場合、そのカードからFinalまで接続する枠を強調できる情報を返す。
- スマホ表示ではR32〜準々決勝をブロックA〜Dに分け、各ブロックを縦型アコーディオンで表示する。
- ブロックA/Bは準決勝1、ブロックC/Dは準決勝2へ接続し、準決勝・決勝は勝者枠として表示する。
- R32の1位・2位通過枠とR16以降の接続順は、確認済みの公式scheduleに基づく明示定数で管理する。
- 3位通過枠の正式なチーム割り当ては未実装・未確定とし、暫定トーナメント表βは非公式表示かつ勝敗予想ではないことを明示する。
